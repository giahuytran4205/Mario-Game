    ...
    m_startY(1000)       // Vị trí dưới màn hình
{
    ...
    mDistanceActiveX = randRange<double>(-110, 100);
    if (std::abs(mDistanceActiveX) < 10)
        mDistanceActiveX += 10;
...
void CheepCheepUnder::update() {
    ...
    const auto posMario = m_mario.getPos();
    auto Post = m_transform.getPosition();

    const auto Distance = -(posMario.x - Post.x);
    const bool isMarioNear = Distance < mDistanceActiveX;
    if (m_autoControl.isControlled())
        return;
    if (isMarioNear && !mIsFall) {
        m_physics.setBaseVelocityX(randRange<double>(0.09, 0.15) * 1);
        m_physics.setBaseVelocityY(randRange<double>(-0.6 ,-0.5) * 1);
        m_autoControl.addWaitForMiliseconds(randRange<int>(3000, 4000));
