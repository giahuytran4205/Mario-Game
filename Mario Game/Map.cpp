...
Map::Map(Mario& mario):mMario(mario) {
	m_width = m_height = 0;
	m_row = m_col = 0;
...
Map::Map(Mario& mario, const string& filename) :mMario(mario) {
	...
	for (auto& i : group["layers"].as_array()) {
		...
		if (layer["name"].as_string() == "Question Block") {
			for (auto& i : layer["objects"].as_array()) {
				...
				//EnemiesCheepCheepNormal* enemiesCheepCheepNormal = new EnemiesCheepCheepNormal(mMario, this);
				//enemiesCheepCheepNormal->getComponent<Transform2D>().setWorldPosition(x + width / 2, y + height / 2);
				////enemiesCheepCheepNormal->getComponent<Transform2D>().setWorldPosition(x + width / 2, 0);
				//m_objects.push_back(enemiesCheepCheepNormal);

				//EnemiesCheepCheepWarter* enemiesCheepCheepWarter = new EnemiesCheepCheepWarter(mMario, this);
